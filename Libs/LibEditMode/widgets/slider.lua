...
function sliderMixin:SetEnabled(enabled)
	...
	-- YIPPYAPP LOCAL PATCH: values are display-only. Upstream lays a
	-- typable InputBoxTemplate over the slider's RightText; we want the
	-- plain formatted number. Everything above is upstream behaviour --
	-- only the EditBox line below is replaced. Re-apply if this lib is
	-- updated (upstream line was: self.EditBox:SetShown(enabled)).
	if self.EditBox then
		self.EditBox:Hide()
		self.EditBox:EnableMouse(false)
	end
	if self.Slider and self.Slider.RightText then
